...
REM              security (Generate security group command for AWS)
...
IF ["%1"]==["security"] SET NEXT=displayHelp & GOTO genSecGroupAWS
...
ECHO %0 location
ECHO.
ECHO Create AWS security group
ECHO %0 security
ECHO.
exit /B 1
...
:genSecGroupAWS
SET sgname=sg%RANDOM%
SET sgdesc=Security group %sgname% generated by %0 %1
SET /P vpc=Enter the VPC ID : 
SET /P sgname=Security group name [%sgname%]: 
SET /P sgdesc=Security group description [%sgdesc%]: 
ECHO aws ec2 create-security-group --group-name %sgname% --description "%sgdesc%" --vpc-id %vpc%
exit /B 1